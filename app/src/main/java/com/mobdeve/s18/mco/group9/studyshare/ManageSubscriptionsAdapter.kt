...
class ManageSubscriptionsAdapter(
    options: FirestoreRecyclerOptions<Course>, private val isSubscribed: Boolean ): FirestoreRecyclerAdapter<Course, ManageSubscriptionsViewHolder>(options) {
    ...
    override fun onBindViewHolder(holder: ManageSubscriptionsViewHolder, position: Int, model: Course) {
        holder.bindData(model, isSubscribed)
